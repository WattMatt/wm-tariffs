-- Fix delete_schematic_meters function to preserve meter readings and documents
-- Only delete visual elements (positions, connections, lines) and orphaned meters
...
RETURNS TABLE(deleted_connections bigint, deleted_positions bigint, deleted_meters bigint)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- This preserves meters that appear in multiple schematics
  WITH meter_ids AS (
    SELECT m.id
    FROM meters m
    WHERE m.id IN (
  ...
  -- Note: Meter readings, CSV files, and documents are NEVER deleted
  -- They remain associated with the meter regardless of schematic cards

  RETURN QUERY SELECT v_deleted_connections, v_deleted_positions, v_deleted_meters;
END;
$$;